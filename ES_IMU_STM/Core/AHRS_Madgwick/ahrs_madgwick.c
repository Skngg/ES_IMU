...
static float beta = betaDef;//sqrt(3/4) * 0.0124;
...
void ToEulerAngles(euler_ang_t * euler)// float q0, float q1, float q2, float q3)
{
	float test = q1*q2 + q3*q0;

	if (test > 0.499 && test < 0.501) { // singularity at north pole
		euler->yaw = 2 * atan2(q1,q0);
		euler->pitch = PI/2;
		euler->roll = 0;
		return;
	}
  ...
  euler->yaw = atan2(2*q2*q0-2*q1*q3 , 1 - 2*sqy - 2*sqz);
	euler->pitch = asin(2*test);
	euler->roll = atan2(2*q1*q0-2*q2*q3 , 1 - 2*sqx - 2*sqz);
}
